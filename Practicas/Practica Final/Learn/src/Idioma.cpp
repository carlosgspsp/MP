...
double Idioma::ranking(const char bg[]){
    ordenar();
    return findBigrama(bg);
}
double Idioma::distancia(Idioma u, Idioma c){
    double distancia = 0;
    for (int i =0 ; i < u.getSize();i++)
    {
        distancia+=(abs(u.ranking(u.getPosicion(i).getBigrama()) - c.ranking(c.getPosicion(i).getBigrama())))/(u.getSize()*u.getSize());
