...
bool Idioma::salvarAFichero(const char *fichero) const {
    ...
    if(fichero1.is_open()){
        fichero1 << "MP-BIGRAMAS_IDIOMA-T-1.0" << endl;
        fichero1 << _idioma << endl;
        fichero1 << _nBigramas << endl;
     ...
     cout << "Error al abrir archivo de guardado";
     return false;
    }
    }
...
void Idioma::ordenar(){
  ordenaDesFrec(_conjunto, _nBigramas);
}
int Idioma::ranking(const char bg[]){
    ordenar();
    int rankin = findBigrama(bg);
    if (rankin ==-1)
       rankin = _nBigramas;
    return rankin;
}
double Idioma::distancia(Idioma c){
    double distancia = 0;
    for (int i =0 ; i < _nBigramas ;i++)
    {
        distancia+=((abs(ranking(_conjunto[i].getBigrama()) - c.ranking(_conjunto[i].getBigrama())))*1.0)/((_nBigramas * _nBigramas)*1.0);
